---
title: "Dynamical-Lexicon-Approach"
...
Nous allons créer une fonction qui va générer un DFM qui va nous servir comme base initiale pour vérifier nos résultats prochainement.
...
```
Maintenant nous transformons en Dcouments-Features-Matrix:

```{r}
data_pol<-dfm(data_pol)
```
...
```{r}
data_pol<-dfm_keep(data_pol, min_nchar = 2)
```

On enlève les "stopwords" donc les mots de ponctuation etc..

```{r}
data_pol<-dfm_remove(data_pol, pattern = stopwords("en"))
```
...
b<-abs(c(rnorm(nb_users, mean=0.4, sd=3)))
b<-round(b)
...
for(i in 2:dim(data_pol)[2]){
  data_users[,i]<-round(abs(c(rnorm(nb_users, mean=0.4, sd=3))))
}
...
```{r}
colnames(data_users)<-colnames(data_pol)
...
Nous avons donc maintenant notre data fixure finale que nous sauvegardons dans notre fichier data:
...
On le sauvegarde dans le fichier data et on retourne ces deux data frame :
...
data<- input
  ...
  data_without_target<-data[, !(colnames(data) %in% c(ID)), drop = FALSE]
```
...
On peut maintenant identifier les poids des mots estimés :
